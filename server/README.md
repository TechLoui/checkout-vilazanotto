...
O checkout aceita um ou dois cartões e segue esta ordem:

1. consulta novamente o preço autoritativo no Artax;
2. confere a soma da divisão e pré-autoriza cada valor na Rede, sem captura;
3. se o segundo cartão recusar, mantém apenas a primeira autorização enquanto
   o hóspede troca o cartão ou pede a liberação do valor;
4. cria uma única reserva no Artax depois que todos os cartões aprovam;
5. captura as transações e lança um pagamento por cartão na mesma reserva;
6. envia e-mail e webhook da Asksuite sem bloquear a resposta.
...
- `MIN_CARD_AMOUNT` define o valor mínimo de cada cartão na divisão (padrão:
  `5`, ou R$ 5,00).
...
| `GET` | `/api/config` | Configuração pública de parcelas e dois cartões |
| `GET` | `/api/availability` | Disponibilidade e preço do Artax |
| `POST` | `/api/checkout` | Um/dois cartões: reserva e cobrança |
| `POST` | `/api/checkout/retry-card` | Troca o cartão recusado numa divisão |
| `POST` | `/api/checkout/cancel-split` | Libera a autorização parcial |
...
Para dividir o pagamento, envie até dois itens em `cards`. Cada cartão possui
seu próprio valor e parcelamento:

```json
{
  "cards": [
    {
      "number": "NUMERO_DO_CARTAO_1",
      "holderName": "MARIA SILVA",
      "expirationMonth": 12,
      "expirationYear": 2030,
      "securityCode": "CVV",
...
A soma é validada em centavos contra o preço autoritativo do Artax antes da
primeira autorização. Dados de cartão nunca são gravados na sessão parcial.
...
- A tentativa parcial de dois cartões também fica em memória por até 30
  minutos. Faça o deploy do backend fora de uma janela de pagamentos ativos.
